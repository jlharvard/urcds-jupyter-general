# FAS Python Jupyter Notebook CS 109b Image
...
To build the image, you can run the included `buildImage.sh` script. This script will build the docker image and tag it with its parent directory name, then run a command to export the installed packages to environment.yml. The commands in the script also specify the platform to build for, which ensures that it works on M1 macs.

After it's been built, this command will start it for testing
- `docker run -it -p 8888:8888 -e DOCKER_STACKS_JUPYTER_CMD=notebook fas-jupyter-cs109b`
...
```yaml
---
title: Jupyter Notebook - CS 109b
cluster: "academic"
attributes:
...
```
huitacademictechnology/fas-jupyter-cs109b:sha-7b5663a -> huitacademictechnology_fas-jupyter-cs109b_sha-7b5663a.sif
```
